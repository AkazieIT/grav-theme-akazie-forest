# v0.2.1
1. [](#bugfix)
    * Theme options / Fonts: uploading a custom font was impossible, `.ttf` and `.woff2` alike. The upload field listed its accepted types as MIME types, but Windows knows none of the font types, so the file dialog ended up offering `*.svgz;*.svg;*.com;*.exe;*.bin` — everything it could resolve from the two non-font entries in that list — and no font could be picked at all. Dragging one in was refused with "You can't upload files of this type", because browsers report no MIME type for font files and the upload widget compares that empty value literally. The field now matches on file extensions (`.ttf`, `.otf`, `.woff`, `.woff2`, upper case included), which the file dialog, the upload widget and Grav's own check all understand. Grav 1.7 and 2.0 alike
2. [](#improved)
    * The same field no longer accepts `application/octet-stream`. Grav reports that type for every extension it does not recognise, so the entry let arbitrary files into `custom/fonts`; the field is now limited to the four font formats it documents. The unused SVG entries were dropped as well — SVG fonts are not rendered by the theme
